...
```text
连接服务器
  └─ 注册 / 登录（邮箱验证码或外部站点账号，由 server-info 决定）
       └─ 登录后用密码自动解锁；必要时输入备用恢复密钥
            └─ 历史
                 ├─ 搜索、日期/设备筛选、详情、复制、删除
                 ├─ 回收站：剩余时间、恢复、永久删除
                 ├─ 设备：名称、平台、最近活动、撤销
...
| 账号 | 登录名或邮箱、密码；邀请/邮箱/外部注册由 server-info 切换；external 模式隐藏本应用注册并给出站点注册链接 |
| 密钥 | 登录后优先用密码信封和本机 CMK 解锁；恢复密钥仅备用；首次初始化仍展示一次恢复码并要求离线保存 |
...
stateDiagram-v2
    ...
    SignedOut --> Locked: 登录成功且尚无本机/密码信封
    SignedOut --> CatchingUp: 登录后密码或本机 CMK 解锁成功
    Locked --> CatchingUp: 恢复密钥解锁
